...
public:
    ...
    ~tx_region() {
        ...
        std::cout << tx_id << " ~tx_region()" << std::endl;
        #if BIG_LOCK
...
private:
    ...
    * */
    ...
    void restart();
    bool try_put(const int key, const int val);
    bool try_get(const int key, int &r);

    chdb *db;
    const int tx_id;
    std::vector<std::pair<int, int>> put_kv;
    struct commands {
        int tp;
        int key;
        int val;

        commands(int tp, int key, int val) : tp(tp), key(key), val(val){}
    };
    std::vector<commands> commands_vec;
    bool prepared = true;
};
